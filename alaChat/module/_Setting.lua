	...
	-->	node method
		...
		local function ColorCallOutButton_OnClick(self)
			if ColorPickerFrame:IsShown() then
				...
				info.r, info.g, info.b = unpack(__db[module][key]);
...
-->	SettingUI	<--
	...
	local function CreateSettingUI()
		...
		local category, _ = Settings.RegisterCanvasLayoutCategory(SettingUIInterfaceOptionsFrameContainer, SettingUIInterfaceOptionsFrameContainer.name);
		category.ID = SettingUIInterfaceOptionsFrameContainer.name
...
SlashCmdList["ALACHAT"] = function()
	Settings.OpenToCategory(SettingUIInterfaceOptionsFrameContainer.name);
end
